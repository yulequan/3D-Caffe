...
#include "caffe/layers/pooling_nd_layer.hpp"
...
void PoolingNDLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  PoolingNDParameter pool_param = this->layer_param_.pooling_nd_param();
  ...
  if (pad_sum != 0) {
     CHECK(this->layer_param_.pooling_nd_param().pool() ==
      PoolingNDParameter_PoolMethod_AVE
      || this->layer_param_.pooling_nd_param().pool() ==
      PoolingNDParameter_PoolMethod_MAX)
...
void PoolingNDLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->layer_param_.pooling_nd_param().pool() ==
      PoolingNDParameter_PoolMethod_MAX && top.size() == 1) {
    max_idx_.Reshape(top_shape);
  }
  // If stochastic pooling, we will initialize the random index part.
  if (this->layer_param_.pooling_nd_param().pool() ==
      PoolingNDParameter_PoolMethod_STOCHASTIC) {
...
// case?
template <typename Dtype>
void PoolingNDLayer<Dtype>::Forward_cpu(
  ...
  switch (this->layer_param_.pooling_nd_param().pool()) {
  case PoolingNDParameter_PoolMethod_MAX:
    // Initialize
    if (use_top_mask) {
    ...
    break;
  case PoolingNDParameter_PoolMethod_AVE:
    ...
    break;
  case PoolingNDParameter_PoolMethod_STOCHASTIC:
    NOT_IMPLEMENTED;
    break;
  default:
